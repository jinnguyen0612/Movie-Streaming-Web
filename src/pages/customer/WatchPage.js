    ...
    <Layout>
      <div className='container mx-auto bg-dry p-6 mb-12'>
        <div className='flex-btn flex-wrap mb-6 gap-2 bg-main rounded border border-gray-800 p-6'>
          <Link to={`/movies/${movie?.title}`} className='md:text-xl text-sm flex gap-3 items-center font-bold text-dryGray'>
            <BiArrowBack/> {movie?.title}
          </Link>
          <div className='flex-btn sm:w-auto w-full gap-5'>
            <button onClick={(e) => handleFavorite(movie.id)} className={`bg-white ${checkFavorite?"text-subMain hover:text-white":"hover:text-subMain text-white"} transitions bg-opacity-50 rounded px-4 py-3 text-sm`}>
              <FaHeart/>
            </button>
            <button onClick={(e) => handleTime()} className={`bg-white text-white hover:text-subMain transitions bg-opacity-50 rounded px-4 py-3 text-sm`}>
              <TbFileTime/>
            </button>
          </div>
        </div>
          ...
          play?(
            ...
            <div className='w-full h-full rounded-lg overflow-hidden relative'>
              <div className='absolute top-0 left-0 bottom-0 right-0 bg-main bg-opacity-30 flex-colo'>
                ...
                <div id="popup-modal" className="z-50 p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                  <div className="relative w-full max-w-md max-h-full">
                    <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 text-black">
                      {/* Nội dung của popup */}
                      <p className='p-5'>Do you want to wath from the last time?</p>
            ...
            <video 
              ...
              <source src={`${movie?.path}`} type='video/mp4' title={movie?.title}/>
            </video>
