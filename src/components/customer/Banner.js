              ...
              <SwiperSlide key={movie.id} className='relative rounded overflow-hidden'>
                ...
                <div className='absolute linear-bg xl:pl-52 sm:pl-32 pl-8 top-0 bottom-0 right-0 left-0 flex flex-col justify-center lg:gap-8 md:gap-5 gap-4'>
                  ...
                  <div className='flex gap-5 items-center'>
                    ...
                    {
                      user?
                      <button onClick={(e)=>handleFavorite(movie.id)} className={`bg-white ${(!isFilmFavorite(movie.id))? "hover:text-subMain text-white": "text-subMain hover:text-white"} transitions  px-4 py-3 rounded text-sm bg-opacity-30`}>
                        <FaHeart/>
                      </button>:
                      <Link to={`/login`} className={`bg-white hover:text-subMain text-white transitions  px-4 py-3 rounded text-sm bg-opacity-30`}>
                        <FaHeart/>
                      </Link>
